 ...
 */
class MyPurchasedListViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var data: LiveData<DataLoadingStatus<MyPurchaseListResponse>> = Transformations.switchMap(reloadTrigger) {
        myListRepository.callForPurchasedVideos(it)
    }
    var message = MutableLiveData<String>().apply {
        ""
    }

    fun getMyList(): LiveData<DataLoadingStatus<MyPurchaseListResponse>> = data
