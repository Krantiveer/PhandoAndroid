...
class WatchLaterListViewHolder(itemView: View, listener: AdapterClickListener) : BaseViewHolder<WatchLaterVideo, AdapterClickListener>(itemView, listener) {

    init {
        itemView.setOnClickListener { v -> listener.onItemClick(v.tag) }

    }
    override fun onBind(item: WatchLaterVideo) {
        itemView.tag = item
