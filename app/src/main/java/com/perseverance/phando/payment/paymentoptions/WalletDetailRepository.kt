...
class WalletDetailRepository {
    ...
    suspend fun getWalletHistory(): WalletHistoryResponseData {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                WalletHistoryResponseData(status = "error",
                    message = "Unable to get wallet history")
            }
    ...
    suspend fun updateOrderOnServer(map: Map<String, String>): BaseResponse {
        val data: MutableLiveData<DataLoadingStatus<BaseResponse>> =
            MutableLiveData<DataLoadingStatus<BaseResponse>>()
