 ...
 */
public class AppTrackSelectionView extends LinearLayout {
     ...
     */
    public AppTrackSelectionView(Context context) {
        this(context, null);
    }
     ...
     * Creates a track selection view.
     */
    public AppTrackSelectionView(Context context, @Nullable AttributeSet attrs) {
    ...
    public AppTrackSelectionView(
        ...
        trackNameProvider = new AppDefaultTrackNameProvider(getResources());
    ...
    private void updateViews() {
        ...
        for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
            ...
            TreeMap<Integer, FormatWrapper> formatTreeMap = new TreeMap<>();
            for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                Format format = group.getFormat(trackIndex);
                formatTreeMap.put(format.width, new FormatWrapper(trackIndex, format));
            }
            ArrayList<FormatWrapper> list = new ArrayList<>(formatTreeMap.size());
            for (Map.Entry<Integer, FormatWrapper> entry : formatTreeMap.entrySet()) {
                Integer key = entry.getKey();
                FormatWrapper value = entry.getValue();
                list.add(value);
            }
            ...
            for (int trackIndex = 0; trackIndex < list.size(); trackIndex++) {
                ...
                FormatWrapper format = list.get(trackIndex);
                ...
                if (mappedTrackInfo.getTrackSupport(rendererIndex, groupIndex, format.getOriginalIndex())
                    ...
                    trackView.setTag(Pair.create(groupIndex, format.getOriginalIndex()));
                    trackView.setOnClickListener(componentListener);
                } else {
                    trackView.setFocusable(false);
                    trackView.setEnabled(false);
                }
                trackViews[groupIndex][format.getOriginalIndex()] = trackView;
                addView(trackView);
            }
        }
    ...
    private class ComponentListener implements OnClickListener {
        ...
        public void onClick(View view) {
            AppTrackSelectionView.this.onClick(view);
        }
    }
}
